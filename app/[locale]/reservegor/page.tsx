...
export default function MyReservationsPage() {
  ...
  const { reservations, isLoading, isError } = usePaginatedReservations(
    user?.id || 0,
    page,
    5
  );
          ...
          <ul className="space-y-3">
            {reservations.map((r: Reservation) => (
              <li
                ...
                <p>
                  {t("start")}: {new Date(r.start_datetime).toLocaleString()}
                </p>
                <p>
                  {t("end")}: {new Date(r.end_datetime).toLocaleString()}
                </p>
                ...
                {visibleOptions[r.id] && (
                  <div className="mt-2">
                    <input
                      ...
                      onChange={(e) =>
                        handleChange(r.id, "start", e.target.value)
                      }
                    ...
                    <input
                      ...
                      value={form[r.id]?.end || ""}
                      onChange={(e) =>
                        handleChange(r.id, "end", e.target.value)
                      }
